		...
		<thead className="">
			<tr className="p-10">
				{Object.keys(data[0]).map((head, index) =>
					<th scope="col" key={index} className="lg:px-5 border py-3 capitalize">
						{head}
					</th>
				)}
		...
		<tbody className="text-xs text-gray-700 uppercase relative">
				<tr className="border-b border-l" key={index}>
					...
					>
						...
						<ModalLayout children={<ProductDetails data={tableData} />} modalButtonClassName="" modalButtonIcon="dotIcon" key={tableData.id} modalButtonName="" />
						</div>
					</td>
				</tr>
				<tr></tr>
